...
本系统为医药销售代表提供 AI 驱动的场景化话术训练环境：销售人员输入话术后，系统通过 4 个专业 Agent 并行分析其发言质量，同时由 AI 模拟客户角色进行回应，最终返回结构化的评分、改进建议和智能引导。此外，系统还会基于 RAG 知识库动态生成 **3 条可点击的推荐话术**（数据导向 / 共情切入 / 利益推动），帮助销售代表快速构思下一轮回复。
...
6. **智能推荐话术** -- 基于 RAG + 动态权重生成 3 条可点击策略话术（数据导向/共情切入/利益推动），直接回应客户最新提问，点击即发送
...
| ResponsePredictor | `response_predictor.py` | 基于 RAG + 动态权重生成 3 条可点击推荐话术 |
...
sequenceDiagram
    ...
    CS-->>WF: ai_response + predicted_responses

    WF-->>API: 完整 WorkflowState
    API->>DB: 保存 AI 回复 + 覆盖记录
    API-->>FE: SendMessageResponse<br/>{ai_response, evaluation, guidance, predicted_responses}
    FE->>FE: 渲染 5 个分析面板 + 推荐话术面板
```
...
| `simulate` | 模拟 | 生成 AI 客户回复 + 3 条推荐话术 | `{ai_response, predicted_responses}` |
| `end` | 结束 | 日志记录 | 空字典 |
...
| `predicted_responses` | `list[dict]` | simulate | router / end | 3 条推荐话术（策略/内容/置信度） |
...
### 5.5 ResponsePredictor（推荐话术生成器）

**源文件**: [response_predictor.py](src/umu_sales_trainer/core/response_predictor.py)

**职责**：基于动态加权 RAG 知识库召回结果和客户最新回复，为销售代表生成 3 条不同策略的可点击推荐话术（Quick Reply）。销售代表可直接点击发送，无需手动打字。

**设计动机**：

在 AI 客户模拟器返回回复后，销售代表需要构思下一轮话术。推荐话术功能通过 RAG 知识库检索 + LLM 生成，提供 3 个不同策略角度的参考话术，降低输入门槛并提升训练效率。

**三种策略**：

| 策略 ID | 中文标签 | 核心特征 | 适用场景 |
|---------|---------|---------|---------|
| `data_evidence` | 数据导向 | 引用临床数据、研究样本量、循证等级，专业严谨 | 产品呈现阶段、客户追问数据时 |
| `empathy_relation` | 共情切入 | 理解医生/患者痛点，拉近距离，温和自然 | 需求探查、异议处理阶段 |
| `benefit_action` | 利益推动 | 强调患者获益、使用场景、行动号召 | 缔结成交阶段 |

**动态加权机制**：

系统根据对话上下文自动调整各知识库 Collection 的检索权重，使推荐话术更贴合当前场景：

| 影响因素 | 权重调整逻辑 |
|---------|-------------|
| 销售阶段 | `opening` → excellent_samples↑ / `presentation` → product_knowledge↑ / `objection_handling` → objection_handling↑ / `closing` → excellent_samples↑ |
| 异议信号 | 检测到 objections 时，objection_handling 额外 +0.3 |
| 客户关注点 | 匹配 HbA1c/低血糖/安全性/依从性 关键词时，对应 Collection 微调 |

**数据流与 Prompt 结构**：

```
workflow._node_simulate()
  │
  ├── ai_response = CustomerSimulator.generate()    ← 先生成 AI 客户回复
  │
  └── predictions = ResponsePredictor.predict(     ← 再生成推荐话术
        sales_message,                              ← 销售代表刚说的话
        last_ai_response=ai_response,               ← ★ 客户最新回复（关键上下文）
        context={...}
      )
```

每条推荐话术的 LLM Prompt 包含以下信息（按优先级排列）：

1. **【客户（张主任）最新回复/提问】** — 最关键，确保话术直接回应客户的关切
2. **【销售代表刚发送的内容】** — 作为上下文参考
3. **【产品知识库参考信息】** — RAG 召回的相关知识（可从中提取数据/案例）
4. **【客户关注点】** — 客户画像中的关注点列表

**输出**：`SuggestedReply` 数据对象

| 字段 | 类型 | 说明 |
|------|------|------|
| `strategy` | `str` | 策略标识符（data_evidence / empathy_relation / benefit_action） |
| `strategy_label` | `str` | 中文标签（"数据导向" / "共情切入" / "利益推动"） |
| `content` | `str` | 推荐话术文本（50-100 字，可直接发送） |
| `confidence` | `float` | 推荐置信度 0-1（基于 RAG 命中质量计算） |
| `source_hints` | `list[str]` | RAG 来源提示（如 ["产品知识库", "异议处理策略"]） |

**前端交互**：

推荐话术以可折叠面板形式呈现在**输入框上方**（chat-input-bar 区域内），默认折叠状态。面板包含 3 张纵向卡片，每张显示策略标签和完整话术内容（CSS `line-clamp: 3` 控制行数）。点击任一卡片即可将话术自动填入输入框并发送。

```mermaid
graph LR
    SIM["AI 客户回复"] --> RP["ResponsePredictor"]
    RAG["RAG 知识库召回"] --> RP
    CTX["对话上下文<br/>stage / objections / concerns"] --> RP
...
前端页面采用左右分栏布局，左侧为 5 个分析面板 + 1 个推荐话术面板，右侧为对话区域：

| 面板 | 内容 | 数据来源 | 位置 |
|------|------|---------|------|
| AI 客户回复面板 | 显示 CustomerSimulator 生成的 AI 客户回应 | `ai_response` | 左侧面板区 |
| 语义覆盖面板 | 显示各语义点的覆盖状态（已覆盖/未覆盖）+ 覆盖率进度条 | `coverage_result` | 左侧面板区 |
| 表达评分面板 | 显示清晰度/专业性/说服力三分项分数 + 改进建议列表 | `expression_result` | 左侧面板区 |
| 会话洞察面板 | 显示当前销售阶段、意图、检测到的异议信号 | `conversation_analysis` | 左侧面板区 |
| 智能引导面板 | 双态显示：优秀态（绿色）/ 改进态（黄色） | `guidance_result` | 左侧面板区 |
| 推荐话术面板 | 3 条可点击策略话术（数据导向/共情切入/利益推动），默认折叠 | `predicted_responses` | 输入框上方 |
...
```json
{
  ...
  "predicted_responses": [
    {
      "strategy": "data_evidence",
      "strategy_label": "数据导向",
      "content": "该数据源自2023年发表于《Diabetes Care》的多中心、双盲、安慰剂对照RCT（NCT04567890），共纳入1,286例T2DM患者...",
      "confidence": 0.72,
      "source_hints": ["产品知识库"]
    },
    {
      "strategy": "empathy_relation",
      "strategy_label": "共情切入",
      "content": "张主任，您提的这些问题特别关键，尤其是对老年和肾功能不全患者的顾虑...",
      "confidence": 0.68,
      "source_hints": ["异议处理策略"]
    },
    {
      "strategy": "benefit_action",
      "strategy_label": "利益推动",
      "content": "张主任，您关注的正是临床最关切的安全性与精准获益问题——糖宁胶囊III期多中心RCT显示...",
      "confidence": 0.65,
      "source_hints": ["优秀话术示例"]
    }
  ]
...
│   │   ├── response_predictor.py   # 推荐话术生成器（Quick Reply）
